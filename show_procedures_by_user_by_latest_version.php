...
$formdata=json_decode(file_get_contents("php://input"));
    ...
    // "SELECT * FROM `procedures` WHERE user_id=".$userId;

$sql="SELECT p.*, u.user_name, CONCAT( u.first_name, ' ' ,u.last_name) AS fullname FROM  `procedures` as p LEFT JOIN user_details as u ON u.user_id = p.user_id inner join ( SELECT proc_no, max(updated_on) as latest from procedures group by proc_no ) u on p.updated_on = u.latest 
        and p.proc_no = u.proc_no where p.user_id='$userId' AND version <> 0 order by updated_on desc";
